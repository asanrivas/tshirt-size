...
const HEADER_KANAK2 = 'kanak2';
const HEADER_HADIR = 'hadir family day';
  ...
  for (let r = 0; r < rows.length; r++) {
    ...
    if (namaCol !== -1 && sizeCol !== -1) {
      return {
        ...
        kanak2Col: row.indexOf(HEADER_KANAK2) + 1,
        hadirCol: row.indexOf(HEADER_HADIR) + 1,
...
/**
 * Headcount of adults vs kanak2, split by whether Hadir Family Day is
 * checked. Rows without a name are skipped; a row with no Hadir Family
 * Day column at all is treated as attending (defaults to TRUE elsewhere).
 */
async function getAttendanceStats() {
  const sheets = getSheetsClient();
  const header = await findHeader(sheets);
  if (!header.kanak2Col) {
    throw new Error('Sheet is missing the Kanak2 column.');
  }

  const namaCol = colToLetter(header.namaCol);
  const kanak2Col = colToLetter(header.kanak2Col);
  const hadirCol = header.hadirCol ? colToLetter(header.hadirCol) : null;

  const cols = [namaCol, kanak2Col, hadirCol].filter(Boolean).sort();
  ...
  const colIndex = (letter) => letter.charCodeAt(0) - firstCol.charCodeAt(0);
  const namaIdx = colIndex(namaCol);
  const kanak2Idx = colIndex(kanak2Col);
  const hadirIdx = hadirCol ? colIndex(hadirCol) : -1;
  ...
  const stats = {
    adultsAttending: 0,
    kidsAttending: 0,
    adultsNotAttending: 0,
    kidsNotAttending: 0
  };

  rows.forEach((r) => {
    const name = String(r[namaIdx] || '').trim();
    if (!name) return;
    const isKid = isTruthy(r[kanak2Idx]);
    const attending = hadirIdx === -1 ? true : isTruthy(r[hadirIdx]);
    if (attending) {
      if (isKid) stats.kidsAttending++; else stats.adultsAttending++;
    } else {
      if (isKid) stats.kidsNotAttending++; else stats.adultsNotAttending++;
    }
  });

  stats.totalAttending = stats.adultsAttending + stats.kidsAttending;
  stats.totalNotAttending = stats.adultsNotAttending + stats.kidsNotAttending;

  return stats;
}

module.exports = { getNames, submitSize, getFamilyReport, getAttendanceStats };
